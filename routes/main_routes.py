...
async def get_ai_response(messages, chat_agent):
...
def chat():
    try:
        ...
        # Get chat agent from app config
        chat_agent = main_bp.chat_agent

        # Get AI response
        ai_response = asyncio.run(get_ai_response(messages, chat_agent))
...
@main_bp.route('/api/data/<path:filename>')
def serve_data_files(filename):
    """Serve static files from the data directory"""
    return send_from_directory('./data', filename)

@main_bp.route('/projects/<project_id>')
@main_bp.route('/api/projects/<project_id>')
